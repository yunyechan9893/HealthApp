...
- 어논테이션 선언
    - @Target() 어논테이션 이름 지정
    - @Retention() : 어논테이션을 어떻게 유지할것인가를 정의함, 자바소스를 컴파일하고 클래스 파일을 생성할 때  어논테이션의 유지정책을 지정함
        - RetentionPolicy.SOURCE: 컴파일 이후 버려진다. 주석같은 용도로 사용
        - RetentionPolicy.CLASS: 컴파일된 클래스 파일에 포함, 런타임시 사용할 수 없음, 컴파일된 클래스 분석 혹은 처리할때 사용
        - RetentionPolicy.RUNTIME: 컴파일된 클래스 파일에 포함, 런타임시 사용가능, 리플렉션을 통해 어논테이션을 읽고 처리할 때 사용
        - Q. 만약 어논테이션을 포함하고 있는 Activity에서 intent를 사용해 다른 Activity로 넘어가고 이전 Activity가 Destroy 되었다면, 어논테이션은 메모리에서 사라지나?
        - A. 아니오. 사라지지 않습니다. 가비지 컬렉터에 의해 회수될 수 있지만 클래스 파일에 저장된 어논테이션 정보는 그대로 유지되기 때문에 리플렉션 등을 사용하여 클래스 파일에서 어논테이션 정보를 읽어올 수 있습니다.


#### 안드로이드
- DBHelper에서 Jetpact Room ORM 방식으로 변경
    - 싱글톤 구현
    - 식단, 음식 테이블 구현
    - select, insert, update, delete
    
#### 백엔드
- sqlAlchemy get_all() 구현
- 식단, 먹었던 음식 테이블 ORM 구현

#### 데이터베이스( MSSQL )
- Food( 음식 칼로리, 탄수화물 등등 컬럼 포함 ) 테이블 삭제
- AteFood 테이블에 Food 테이블 통합
- Diet 테이블 이미지 Url추가
...
#### 안드로이드
- mainActivity 리팩토링
    - 단독 Fragment관리에서 FragmentStateAdapter를 이용하여 관리
    - 코드 최적화
- SQL Lite 사용
- homeFragment 리팩토링
    - 코드 최적화
